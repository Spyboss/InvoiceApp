...
Before (invoice_app.py:19, 86, 211)
```
...
```
Call after PDF build (invoice_app.py:397–401)
```
...
Before (constructor button area invoice_app.py:348–349)
```
...
- Replace folder/out_path lines with the `os.path.join` version and add `safe_filename`.
